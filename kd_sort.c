...
inline int compare_ge( const void *A, const void *B, int i )
{
...
inline int partitioning( struct kd_node_t *data, int start, int end, compare_t cmp_ge, int i )
{ 
  void *pivot = (void*)&data[end];
  
  int pointbreak = end-1;
  for ( int i = start; i <= pointbreak; i++ )
    if( cmp_ge( (void*)&data[i], pivot, i ) )
      {
	while( (pointbreak > i) && cmp_ge( (void*)&data[pointbreak], pivot, i ) ) pointbreak--;
	if (pointbreak > i ) 
	  SWAP( (void*)&data[i], (void*)&data[pointbreak--], sizeof(struct kd_node_t) );
      }  
  pointbreak += !cmp_ge( (void*)&data[pointbreak], pivot, i ) ;
...
void pqsort( struct kd_node_t *data, int start, int end, compare_t cmp_ge, int i)
{
  int size = end-start;
  if ( size > 2 )
    {
      int mid = partitioning( data, start, end, cmp_ge, i );
      
     #pragma omp task shared(data) firstprivate(start, mid)
      pqsort( data, start, mid, cmp_ge,i );
     #pragma omp task shared(data) firstprivate(mid, end)
      pqsort( data, mid+1, end , cmp_ge,i );
    }
  else
    {
      if ( (size == 2) && cmp_ge ( (void*)&data[start], (void*)&data[end-1], i ) )
